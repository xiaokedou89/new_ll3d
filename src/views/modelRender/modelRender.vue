...
export default {
  ...
  methods: {
    ...
    // 初始化音乐素材
    initMusic() {
      materialApi.getMaterialList('voice', this.musicParams)
        .then(res => {
          console.log('音乐素材数据');
          console.log(res);
          if (res.code === 20000) {
    ...
    // 拉取图片素材
    initImages() {
      materialApi.getMaterialList('image', this.imageParams)
        .then(res => {
          console.log('图片素材');
          console.log(res);
          if (res.code === 20000) {
    ...
    // 拉取天空盒图片素材
    initSkyboxs() {
      materialApi.getMaterialList('vrPanormaImage', this.skyboxParams)
        .then(res => {
          console.log('天空盒图片素材');
          console.log(res);
          if (res.code === 20000) {
    ...
    thumbBeforeUpload(file) {
      ...
      let extension = file.name.substring(file.name.lastIndexOf('.') + 1),
          size = file.size,
          extensionFlag = (extension === 'jpg' || extension === 'png' || extension === 'jpeg') && size <= 5 * 1024 * 1024 ? true : false;
      if (!extensionFlag) {
        this.$message({
    ...
    // 打开天空盒图片素材弹框
    openSkyboxDialog() {
      console.log(this.configObj.skybox);
      console.log(this.skyboxs);
      this.setSkyboxSelectDisabled();
