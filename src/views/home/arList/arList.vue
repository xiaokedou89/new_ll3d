<template>
  <div class="main">
    ...
    <div class="container container-padding">
      ...
      <div class="condition-container">
        ...
        <div class="radio-condition">
          ...
          <dropdown-menu class="radio-item" :placement="'bottom'" :dropable="true">
             <template v-slot:content>
              ...
              <div class="radio-container">
                ...
                <!-- 中间的类型名称 -->
                <span class="condition-title">{{ listType.name }}</span>
                <!-- 右边的icon -->
                <i class="iconfont condition-icon-right icon-sanjiaodown"></i>
              </div>
            </template>
            <li v-for="(item, index) in types" :key="index" @click.stop="changeType(item)">
              <span>{{ item.name }}</span>
            </li>
          </dropdown-menu>
          <!-- 日期 -->
          ...
          <!-- 是否下载 -->
          <dropdown-menu class="radio-item" :placement="'bottom'" :dropable="true" v-if="false">
             <template v-slot:content>
              <!-- 显示在条件部分的部分 -->
              <div class="radio-container">
                <!-- 左边的icon -->
    ...
    <!-- 主列表部分 -->
    <list :user="user" :loading="loading" :list="list" @itemClick="itemClick" @btnClick="btnClick"></list>
  </div>
</template>
<script>
import list from '@/components/layout/list/list.vue';
import { getVrList } from '@/api/vr/vr.js';
import { modelListParams, listSize, arListTypes } from '@/utils/global.js';
...
export default {
  ...
  methods: {
    ...
    initList(params) {
      this.loading = true;
      // getVrList(modelListParams.type.MODEL,params)
      getVrList(this.listType.value, params)
        .then(res => {
          if (res.code === 20000) {
    ...
    // 点击列表项
    itemClick(item) {
      let path = item.type ? '/vrmodelEditor' : '/panEditor';
      this.$router.push({
        path,
        query: {
          id: item.id,
