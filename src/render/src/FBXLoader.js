...
const { Zlib } =require('@/utils/zlib.min')
// console.log(Zlib)
import { NURBSCurve } from 'three/examples/jsm/curves/NURBSCurve.js';
import {
    Group, TextureLoader, Quaternion, Loader, ClampToEdgeWrapping, SkinnedMesh, Euler, PerspectiveCamera, Math, Texture, SpotLight, BufferAttribute, VertexColors, PropertyBinding, MeshStandardMaterial, DirectionalLight, Skeleton, LoadingManager, LineBasicMaterial, RepeatWrapping, MeshLambertMaterial, EquirectangularReflectionMapping, Color, OrthographicCamera, Mesh, Material, Vector3, Vector4, Matrix4, Object3D, MeshPhongMaterial, Line, Vector2, Uint16BufferAttribute,Float32BufferAttribute, DefaultLoadingManager, FileLoader, Bone, MultiMaterial, PointLight, AnimationClip, BufferGeometry, AmbientLight,LoaderUtils,Matrix3,VectorKeyframeTrack
} from "three";//js/es*/load*/path
    ...
    Object.assign( FBXLoader.prototype, {
        ...
        load: function ( url, onLoad, onProgress, onError,urlMap ) {
            ...
            loader.load( url, function ( buffer ) {
                ...
                try {
                    ...
                    var scene = self.parse( buffer, resourceDirectory,urlMap );
                    onLoad( scene );
        ...
        parse: function ( FBXBuffer, resourceDirectory ,urlMap) {
            ...
            var textures = parseTextures( FBXTree, new TextureLoader( this.manager ).setPath( resourceDirectory ), images, connections ,urlMap);
    ...
    function parseTextures( FBXTree, loader, images, connections,urlMap ) {
        ...
        if ( 'Texture' in FBXTree.Objects ) {
            ...
            for ( var nodeID in textureNodes ) {
                ...
                var texture = parseTexture( textureNodes[ nodeID ], loader, images, connections,urlMap );
...
//此处加上贴图映射filePath
    function parseTexture( textureNode, loader, images, connections,urlMap ) {

        var texture = loadTexture( textureNode, loader, images, connections,urlMap );
    ...
    function loadTexture( textureNode, loader, images, connections,urlMap ) {
        ...
        if (urlMap){
            urlMap.forEach(function (one) {
                if (filePath.indexOf(one.oldFileName)!=-1){
